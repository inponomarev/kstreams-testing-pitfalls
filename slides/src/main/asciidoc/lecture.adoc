...
digraph G {
  ...
  processor [shape="septagon"; label="\nTopology\n "];
...
digraph G {
  ...
  source [shape=none;
  label=<<table cellborder="0" padding="0" border="1">
  <tr><td>K</td><vr/><td>V</td><vr/><td><IMG SCALE="BOTH" SRC="slides/src/main/asciidoc/images/stopwatch.png"/></td></tr>
  <hr/>
  <tr><td>K</td><vr/><td>V</td><vr/><td><IMG SCALE="BOTH" SRC="slides/src/main/asciidoc/images/stopwatch.png"/></td></tr>
  <hr/>
  <tr><td>K</td><vr/><td>V</td><vr/><td><IMG SCALE="BOTH" SRC="slides/src/main/asciidoc/images/stopwatch.png"/></td></tr>
  ...
  source->processor[label="pipeInput(V, Instant)\lpipeInput(K, V, Instant)";fontname="Courier"];
  processor->sink[label="";fontname="Courier"];
}
----
...
* poll
* initialize/restore (idempotent)
* process (batch) (cached)
* punctuate
* flush caches
* commit

[graphviz,"streamsLoop.png"]
----
digraph G {
rankdir="LR";
start [];
runLoop [label="while running", ];
poll [];
process [];
cache [];
forward [];
...
start -> runLoop;
runLoop -> poll [arrowhead="dot"];
poll -> process [arrowhead="dot"];
process -> forward;
process -> cache;
forward -> send;
process -> runLoop [arrowhead="dot"];
commit -> flush;
flush -> forward;
runLoop -> commit [arrowhead="dot", label="on commit interval", color="grey", fontcolor="grey"];
}
----
...
** punctuate
** flush caches
** re-enqueue internal "topic" results

[graphviz,"ttdLoop.png"]
----
digraph G {
rankdir="LR";
start [label="enqueue"];
runLoop [label="while queue not empty", ];
dequeue [];
poll [];
process [];
forward [];
...
start -> runLoop;
runLoop -> dequeue [arrowhead="dot"];
dequeue -> poll [arrowhead="dot"];
poll -> process [arrowhead="dot"];
process -> forward;
forward -> enqueue;
enqueue -> runLoop [arrowhead="dot"];
}
----

== Other problems that can't be surfaced with `TopologyTestDriver`

* "Single partition" -- everything related to co-partitioning
* "Fused" subtopologies -- everything related to repartition operations
* Timing: 
** stream-stream joins can behave differently
